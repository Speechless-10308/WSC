...
def main(args):
    ...
    train_data, train_targets, test_data, test_targets, _ = get_data(
        args.data_path, args.dataset
    )

    # get semi-supervise data
    args.logger.info(f"labeled data number: {args.num_labels}")
    lb_train_data, lb_train_targets, ulb_train_data, ulb_train_targets = (
        get_semisup_labels(
            train_data,
            train_targets,
    ...
    train_lb_dataset = ImgThreeViewDataset(
        args, lb_train_data, lb_train_targets, "semi"
    )
    train_ulb_dataset = ImgThreeViewDataset(
        args, ulb_train_data, ulb_train_targets, "semi"
    )
    ...
    model = model.cuda()
    args.logger.info(
        f"Model {args.model} created, with {count_parameters(model)} trainable parameters"
    )
    ...
    if args.layer_decay != 1.0:
        ...
        per_param_args = param_groups_weight_decay(
            model, args.weight_decay, no_weight_decay_list=no_decay
        )
    ...
    if args.optimizer == "SGD":
        ...
        optimizer = torch.optim.AdamW(
            per_param_args, lr=args.lr, weight_decay=args.weight_decay
        )
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        args.logger.info(
            f"Epoch {epoch + 1}: LR: {optimizer.param_groups[0]['lr']:.4f} Train Loss: {train_loss:.4f} L1 Loss: {l1:.4f} L2 Loss: {l2:.4f} Val Loss: {val_loss:.4f} Val Acc: {val_acc:.4f}"
        )
...
def train(
    ...
    for (lb_w, lb_s, lb_s_, lb_y), (ulb_w, ulb_s, ulb_s_, _) in progress_bar:
        ...
        wsc_loss, l1, l2 = weak_spec_loss(
            feat_lb_s, feat_lb_s_, probs_lb_w, feat_ulb_s, feat_ulb_s_
        )
