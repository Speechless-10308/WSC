...
class WideResNetCon(nn.Module):
    ...
    def __init__(self, num_classes, depth=28, widen_factor=2, drop_rate=0.0):
        ...
        print(feat_dim)
        self.head = nn.Sequential(nn.Linear(channels[3], 4 * channels[3]), nn.BatchNorm1d(4 * channels[3]),
                                  nn.ReLU(inplace=True),
                                  nn.Linear(4 * channels[3], feat_dim))
...
class WideResNetConPre(nn.Module):

    def __init__(self, num_classes, depth=28, widen_factor=2, drop_rate=0.0):
        super(WideResNetConPre, self).__init__()
        channels = [16, 16 * widen_factor, 32 * widen_factor, 64 * widen_factor]
        assert ((depth - 4) % 6 == 0)
        n = (depth - 4) / 6
        block = BasicBlock
